...
- 「升厂级」按钮只给创建人，规格是：覆盖创建路径的工艺工程师即可升档（矩阵 8.1），创建人以外的 PE 在页面上点不到。
...
- **允许升档**：仅草稿且当前为否。
- **升厂级**：`工艺工程师` 且作用域覆盖创建节点（直属须整厂作用域），且可用+可复制；不再要求「必须是创建人」。
- **正文**：个人级只创建人；厂级草稿给工艺工程师/超管；可用厂级操作员也可读。点了却 403 的按钮不画。
