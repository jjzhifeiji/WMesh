...
function canCopyInto(src: FSNode, dest: FSNode, byId: Map<string, FSNode>) {
  ...
  if (src.assetKind !== dest.assetKind || !sameTree(src, dest)) return false;
  return src.nodeKind !== "folder" || !isAncestor(src.id, dest.id, byId);
}
...
export function ProcessExplorer({
  ...
  const canWrite = (n: FSNode) => Boolean(writable(n) && !readOnly);
  const allowClip = !readOnly && !factoryId;
  ...
  const copyNode = (n: FSNode) => {
    if (!allowClip || !canCopyNode(n)) return;
    setClipId(n.id);
    message.success("已复制");
  };
  ...
  const pasteInto = (dest: FSNode) => {
    ...
    if (!clip || !canWrite(dest) || !canCopyInto(clip, dest, byId)) return;
    copy.mutate(
  ...
  const pasteItem = (dest: FSNode | null) => {
    if (!allowClip) return null;
    const clip = clipId ? byId.get(clipId) : undefined;
    const ok = Boolean(dest && canWrite(dest) && clip && canCopyInto(clip, dest, byId));
    return {
      key: "paste",
      label: "粘贴",
      disabled: !ok,
  ...
  const moveTo = (n: FSNode) => {
    const folders = nodes.filter((x) => x.nodeKind === "folder" && canMoveInto(n, x, byId, canWrite));
  ...
  const hoverDest = (dest: FSNode, e: DragEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (dragSrc && canMoveInto(dragSrc, dest, byId, canWrite)) {
  ...
  const dropInto = (dest: FSNode, e: DragEvent) => {
    ...
    endDrag();
    if (!src || !canMoveInto(src, dest, byId, canWrite)) return;
    move.mutate(
  ...
  const fileMenu = (n: FSNode): MenuProps["items"] => {
    ...
    return [
      ...
      allowClip ? { type: "divider" } : null,
      allowClip ? { key: "copy", label: "复制", disabled: !canCopyNode(n), onClick: () => copyNode(n) } : null,
      pasteItem(current),
  ...
  const onExplorerKey = (e: KeyboardEvent<HTMLDivElement>) => {
    ...
    if (allowClip && (e.ctrlKey || e.metaKey) && e.key.toLowerCase() === "c") {
    ...
    if (allowClip && (e.ctrlKey || e.metaKey) && e.key.toLowerCase() === "v") {
      if (current) {
        e.preventDefault();
        pasteInto(current);
      }
    ...
    if (e.key === "F2" && focus?.nodeKind === "folder" && focus.parentId && canWrite(focus)) {
      e.preventDefault();
      renameFolder(focus);
      return;
    }
    if (e.key === "Delete" && focus?.parentId && canWrite(focus)) {
      e.preventDefault();
      deleteNode(focus);
    }
  };
      ...
      return {
        ...
        title: n ? (
          <TreeTitle
            ...
            over={overId === n.id}
            canDrag={Boolean(n.parentId && canWrite(n))}
            ...
            {shown.length === 0 ? (
              ...
              shown.map((n) => (
                <button
                  ...
                  type="button"
                  data-fs-id={n.id}
                  draggable={Boolean(!searching && n.parentId && canWrite(n))}
                  className={`fs-row${n.id === listFocusId || (n.assetId && n.assetId === selectedAssetId) ? " is-on" : ""}${overId === n.id && n.nodeKind === "folder" ? " is-drop" : ""}${dragId === n.id ? " is-drag" : ""}${!searching && n.parentId && canWrite(n) ? " is-grab" : ""}`}
                  onDragStart={(e) => beginDrag(n, e)}
                  onDragOver={(e) => {
