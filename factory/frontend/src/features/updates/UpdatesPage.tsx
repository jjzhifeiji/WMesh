import { App, Button, Card, Modal, Popconfirm, Progress, Space, Table, Tag, type TableColumnsType } from "antd";
import { useState } from "react";
import { errorMessage } from "@/shared/api/client";
import { formatBytes, formatTime, shortHash } from "@/shared/format";
import {
  softwareKindLabel,
  useCurrentFactorySoftware,
  useDeleteFactorySoftware,
  useFactorySoftware,
  useImagePrune,
  usePruneFactorySoftware,
  useRequestImagePrune,
  useStorageUsage,
  type FactorySoftwareKind,
  type FactorySoftwareRow,
  type ImageItem,
} from "./api";

const kinds: FactorySoftwareKind[] = ["factory_service", "client_apk"];
const localKind: FactorySoftwareKind = "factory_service";

type KindSummary = {
  kind: FactorySoftwareKind; // 厂服务 / 客户端
  latest?: FactorySoftwareRow; // 本厂已收最高版本
  installed?: FactorySoftwareRow; // 正在跑的厂服务
  total: number; // 已收份数
  old: number; // 可清份数
};

function keepTag(keep?: string) {
  if (keep === "installed") return <Tag color="green">已装</Tag>;
  if (keep === "latest") return <Tag color="blue">当前最高</Tag>;
  return <Tag>可清理</Tag>;
}

function imageKeepTag(keep?: string) {
  if (keep === "current") return <Tag color="green">当前</Tag>;
  if (keep === "previous") return <Tag color="blue">上一份</Tag>;
  return <Tag>可清理</Tag>;
}

function latestOf(rows: FactorySoftwareRow[]) {
  return rows.reduce<FactorySoftwareRow | undefined>((best, row) => {
    if (!best || row.version > best.version) return row;
    return best;
  }, undefined);
}

// 嵌在概览里：每种只留最新一条，详情里看全部副本、本机镜像并清理。
export function UpdatesPanel() {
  const { message } = App.useApp();
  const current = useCurrentFactorySoftware();
  const list = useFactorySoftware();
  const del = useDeleteFactorySoftware();
  const prune = usePruneFactorySoftware();
  const requestPrune = useRequestImagePrune();
  const storage = useStorageUsage();
  const [pruneNonce, setPruneNonce] = useState(0);
  const [pruneWait, setPruneWait] = useState(false);
  const [detail, setDetail] = useState<FactorySoftwareKind | null>(null);
  const image = useImagePrune(pruneWait, pruneNonce);
  const disk = storage.data?.disk;
  const diskPct = disk && disk.total > 0 ? Math.min(100, Math.round((disk.used / disk.total) * 100)) : 0;
  const images = storage.data?.images;
  const imageItems = images?.items ?? [];
  const imageOld = imageItems.filter((row) => !row.keep).length;
  const imageText = images?.kind ? `${images.count} 个　${formatBytes(images.used)}` : "还没有回报";
  const pruneText = image.data?.ready
    ? image.data.ok
      ? `已释放 ${image.data.reclaimed || "空间"}`
      : "清理镜像失败"
    : pruneWait
      ? "清理中…"
      : pruneNonce > 0
        ? "本机还没回报"
        : null;

  const rows = list.data ?? [];
  const summaries: KindSummary[] = kinds.map((kind) => {
    const data = rows.filter((r) => r.kind === kind);
    return {
      kind,
      latest: latestOf(data),
      installed: data.find((r) => r.keep === "installed"),
      total: data.length,
      old: data.filter((r) => !r.keep).length,
    };
  });
  const detailRows = detail ? rows.filter((r) => r.kind === detail) : [];
  const detailOld = detailRows.filter((r) => !r.keep).length;
  const detailImages = detail === localKind;

  const onPruneImages = async () => {
    try {
      await requestPrune.mutateAsync();
      setPruneWait(true);
      setPruneNonce((n) => n + 1);
      window.setTimeout(() => setPruneWait(false), 30_000);
    } catch (e) {
      message.error(errorMessage(e));
    }
  };

  const summaryCols: TableColumnsType<KindSummary> = [
    { title: "种类", dataIndex: "kind", width: 120, render: (k: FactorySoftwareKind) => softwareKindLabel[k] },
    {
      title: "最新版本",
      key: "latest",
      render: (_, row) =>
        row.latest ? (
          <Space>
            <span>
              {row.latest.versionName}（{row.latest.version}）
            </span>
            {keepTag(row.latest.keep)}
          </Space>
        ) : (
          "本厂还没有收到这个种类的包"
        ),
    },
    {
      title: "本厂已装",
      key: "installed",
      width: 180,
      render: (_, row) =>
        row.kind !== "factory_service"
          ? "—"
          : row.installed
            ? `${row.installed.versionName}（${row.installed.version}）`
            : current.data?.version
              ? `${current.data.versionName}（${current.data.version}）`
              : "未装",
    },
    { title: "本厂已收", dataIndex: "total", width: 90, render: (n: number) => `${n} 份` },
    {
      title: "镜像",
      key: "images",
      width: 160,
      render: (_, row) => (row.kind === localKind ? imageText : "—"),
    },
    {
      title: "",
      key: "act",
      width: 90,
      render: (_, row) => (
        <Button type="link" size="small" disabled={row.kind !== localKind && row.total === 0} onClick={() => setDetail(row.kind)}>
          详情
        </Button>
      ),
    },
  ];

  const detailCols: TableColumnsType<FactorySoftwareRow> = [
    { title: "版本号", dataIndex: "version", width: 90 },
    { title: "版本名", dataIndex: "versionName" },
    { title: "摘要", dataIndex: "digest", width: 140, render: (v: string) => shortHash(v) },
    { title: "收到时间", dataIndex: "receivedAt", width: 170, render: (v: string) => formatTime(v) },
    { title: "状态", dataIndex: "keep", width: 110, render: (v: string) => keepTag(v) },
    {
      title: "",
      key: "act",
      width: 90,
      render: (_, row) =>
        row.keep ? null : (
          <Popconfirm
            title="删掉这个本厂旧包？"
            okText="删除"
            cancelText="取消"
            onConfirm={async () => {
              try {
                await del.mutateAsync({ kind: row.kind, version: row.version });
              } catch (e) {
                message.error(errorMessage(e));
              }
            }}
          >
            <Button type="link" size="small" danger>
              删除
            </Button>
          </Popconfirm>
        ),
    },
  ];

  const imageCols: TableColumnsType<ImageItem> = [
    { title: "标签", dataIndex: "ref" },
    { title: "占用", dataIndex: "size", width: 120, render: (n: number) => formatBytes(n) },
    { title: "状态", dataIndex: "keep", width: 110, render: (v: string) => imageKeepTag(v) },
  ];

  return (
    <>
      <Space direction="vertical" size={16} style={{ width: "100%" }}>
        <Card title="软件更新">
          <Table<KindSummary>
            rowKey="kind"
            columns={summaryCols}
            dataSource={summaries}
            loading={list.isLoading}
            pagination={false}
          />
        </Card>
        <Card title="本机存储" loading={storage.isLoading}>
          <Space direction="vertical" size={12} style={{ width: "100%" }}>
            <div>
              <div>
                本机磁盘　已用 {formatBytes(disk?.used ?? 0)} / 共 {formatBytes(disk?.total ?? 0)}，可用 {formatBytes(disk?.avail ?? 0)}
              </div>
              <Progress percent={diskPct} size="small" status={diskPct >= 90 ? "exception" : undefined} />
            </div>
            <div>
              对象存储　已用 {formatBytes(storage.data?.oss.used ?? 0)}（{storage.data?.oss.objects ?? 0} 个对象）
              {storage.data?.oss.bucket ? `　${storage.data.oss.bucket}` : ""}
            </div>
            <div>本厂库　已用 {formatBytes(storage.data?.database ?? 0)}</div>
            <div>本机镜像　{images?.kind ? `已用 ${formatBytes(images.used)}（${images.count} 个）` : "还没有回报"}</div>
          </Space>
        </Card>
      </Space>
      <Modal
        title={detail ? `${softwareKindLabel[detail]}全部版本` : "全部版本"}
        open={detail !== null}
        onCancel={() => setDetail(null)}
        footer={null}
        width={880}
        destroyOnHidden
        styles={{ body: { maxHeight: "60vh", overflow: "auto" } }}
      >
        <Space style={{ marginBottom: 12 }}>
          <Popconfirm
            title="清掉该种类所有可删旧包？"
            okText="清理"
            cancelText="取消"
            disabled={detailOld === 0}
            onConfirm={async () => {
              if (!detail) return;
              try {
                const got = await prune.mutateAsync(detail);
                message.success(got.deleted ? `已清理 ${got.deleted} 份` : "没有可清理的旧包");
              } catch (e) {
                message.error(errorMessage(e));
              }
            }}
          >
            <Button size="small" disabled={detailOld === 0} loading={prune.isPending}>
              清理旧版本
            </Button>
          </Popconfirm>
          <span>{detailOld ? `${detailOld} 份可清` : "没有可清理的旧包"}</span>
        </Space>
        <Table<FactorySoftwareRow>
          rowKey={(r) => `${r.kind}:${r.version}`}
          columns={detailCols}
          dataSource={detailRows}
          pagination={{ pageSize: 10, hideOnSinglePage: true }}
        />
        {detailImages ? (
          <Space direction="vertical" size={12} style={{ width: "100%", marginTop: 16 }}>
            <Space>
              <Popconfirm
                title="清掉不再用的 app 镜像？"
                okText="清理"
                cancelText="取消"
                onConfirm={() => void onPruneImages()}
              >
                <Button size="small" loading={requestPrune.isPending}>
                  清理无用镜像
                </Button>
              </Popconfirm>
              <span>{imageOld ? `${imageOld} 个可清` : "没有可清理的镜像"}</span>
              {pruneText ? <span>{pruneText}</span> : null}
            </Space>
            <Table<ImageItem>
              rowKey={(r) => `${r.ref}:${r.id}`}
              columns={imageCols}
              dataSource={imageItems}
              pagination={false}
              locale={{ emptyText: images?.kind ? "本机没有这类镜像" : "本机还没有回报镜像占用" }}
            />
          </Space>
        ) : null}
      </Modal>
    </>
  );
}
