...
var (
	...
	ErrDeviceSerialRequired  = errors.New("device serial is required")              // 登记/登录必须读到机械臂识别号
	ErrDeviceSerialTaken     = errors.New("device serial already bound")            // 本厂另一台未作废 Client 已钉该号
	ErrDeviceSerialMismatch  = errors.New("device serial does not match")           // 读到的号与这台已钉的不一致
