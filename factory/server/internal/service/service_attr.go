...
func (s *Service) CreateFact(ctx context.Context, token string, wc WorkContext) (FactStub, error) {
...
func (s *Service) CreatePersonalAsset(ctx context.Context, token string, wc WorkContext, content string) (PersonalAsset, error) {
	acc, err := s.RequireActive(ctx, token)
	if err != nil {
...
func (s *Service) GetFact(ctx context.Context, token string, factID uuid.UUID) (FactStub, error) {
	acc, err := s.RequireActive(ctx, token)
	if err != nil {
		return FactStub{}, err
	}
...
func (s *Service) ListFactsByCreator(ctx context.Context, token string, creatorID uuid.UUID) ([]FactStub, error) {
	acc, err := s.RequireActive(ctx, token)
	if err != nil {
		return nil, err
	}
...
func (s *Service) GetPersonalAsset(ctx context.Context, token string, assetID uuid.UUID) (PersonalAsset, error) {
	acc, err := s.RequireActive(ctx, token)
	if err != nil {
		return PersonalAsset{}, err
	}
...
func (s *Service) ReadPersonalAssetContent(ctx context.Context, token string, assetID uuid.UUID) (string, error) {
	acc, err := s.RequireActive(ctx, token)
	if err != nil {
		return "", err
	}
...
func (s *Service) TransferPersonalAsset(ctx context.Context, token string, assetID, to uuid.UUID) error {
	acc, err := s.RequireActive(ctx, token)
	if err != nil {
		return err
	}
...
func (s *Service) RewriteFactPath(ctx context.Context, token string, factID uuid.UUID) error {
	acc, err := s.RequireActive(ctx, token)
	if err != nil {
		return err
	}
...
func (s *Service) RenameOrgUnit(ctx context.Context, token string, unitID uuid.UUID, name string) error {
	acc, err := s.RequireActive(ctx, token)
	if err != nil {
		return err
	}
...
func (s *Service) resolveWorkContext(ctx context.Context, acc Account, wc WorkContext) (*uuid.UUID, []PathNode, error) {
...
func (s *Service) canOperateFactory(ctx context.Context, acc Account) error {
...
func (s *Service) canReadMeta(ctx context.Context, acc Account, creatorID uuid.UUID, unitID *uuid.UUID) error {
	if acc.ID == creatorID {
		return nil
	}
