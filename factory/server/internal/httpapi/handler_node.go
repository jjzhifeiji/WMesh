...
func (h *Handler) listClients(w http.ResponseWriter, r *http.Request) {
	h.withFactory(w, r, func(svc *service.Service) {
		rows, err := svc.ListClients(r.Context(), bearer(r))
		if err != nil {
			writeErr(w, err)
			return
		}
...
func (h *Handler) registerClient(w http.ResponseWriter, r *http.Request) {
	h.withFactory(w, r, func(svc *service.Service) {
		...
		row, err := svc.RegisterBinding(r.Context(), bearer(r), cid, pub, req.BindingRevision)
		if err != nil {
			writeErr(w, err)
			return
		}
...
func (h *Handler) voidClient(w http.ResponseWriter, r *http.Request) {
	h.withFactory(w, r, func(svc *service.Service) {
		...
		if err := svc.VoidClientBinding(r.Context(), bearer(r), clientID); err != nil {
			writeErr(w, err)
			return
		}
...
func (h *Handler) writeRuntime(w http.ResponseWriter, r *http.Request, canRun bool) {
	h.withFactory(w, r, func(svc *service.Service) {
		...
		if canRun {
			cred, err = svc.IssueRuntimeGrant(r.Context(), bearer(r), clientID, nb, na)
		} else {
			cred, err = svc.RevokeRuntimeGrant(r.Context(), bearer(r), clientID, nb, na)
		}
		if err != nil {
			writeErr(w, err)
			return
...
func (h *Handler) listRuntime(w http.ResponseWriter, r *http.Request) {
	h.withFactory(w, r, func(svc *service.Service) {
		rows, err := svc.ListRuntimeGrants(r.Context(), bearer(r))
		if err != nil {
			writeErr(w, err)
			return
		}
...
func (h *Handler) issuePerson(w http.ResponseWriter, r *http.Request) {
	h.withFactory(w, r, func(svc *service.Service) {
		...
		cred, err := svc.IssuePersonOfflineGrant(r.Context(), bearer(r), personID, clientID, nb, na)
		if err != nil {
			writeErr(w, err)
			return
		}
...
func (h *Handler) listPersonGrants(w http.ResponseWriter, r *http.Request) {
	h.withFactory(w, r, func(svc *service.Service) {
		rows, err := svc.ListPersonGrantViews(r.Context(), bearer(r))
		if err != nil {
			writeErr(w, err)
			return
		}
...
func (h *Handler) signingPublicKey(w http.ResponseWriter, r *http.Request) {
	h.withFactory(w, r, func(svc *service.Service) {
		if _, err := svc.RequireActive(r.Context(), bearer(r)); err != nil {
			writeErr(w, err)
			return
		}
		pub, err := svc.SigningPublicKey(r.Context())
		if err != nil {
			writeErr(w, err)
			return
		}
