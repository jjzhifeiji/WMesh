...
func (h *Handler) importLegacy(w http.ResponseWriter, r *http.Request) {
	h.withFactory(w, r, func(svc *service.Service) {
		...
		for _, f := range req.Processes {
			in.Processes = append(in.Processes, service.LegacyFile{Path: f.Path, Name: f.Name, Content: []byte(f.Content)})
		}
		for _, f := range req.Projects {
			in.Projects = append(in.Projects, service.LegacyFile{Path: f.Path, Name: f.Name, Content: []byte(f.Content)})
		}
