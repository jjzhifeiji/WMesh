...
abstract class WeldingViewModel(
    ...
    protected data class Vector3(val x: Double, val y: Double, val z: Double) {
    ...
    protected var downloadId: Long = -1L
    ...
    protected var weldingTimerJob: kotlinx.coroutines.Job? = null

    protected var alarmVoiceJob: kotlinx.coroutines.Job? = null

    protected var lastReachedPoint: WeldPoint? = null

    protected var lastReachedPointIndex: Int = -1

    protected var lastReachedPathIndex: Int = -1

    // ID -> Triple(PathIndex, PointIndex, ExtraProcessIndex) ExtraProcessIndex=-1 表示主工艺

    protected val commandIdMap = mutableMapOf<Int, Triple<Int, Int, Int>>()

    protected val commandLineMap = mutableMapOf<Int, Int>()

    protected var globalCommandId = 1000
    ...
    protected data class ExecutionGroup(
        val commandString: String,
        val lastCommandId: Int,
        val description: String
    )

    protected val executionQueue = ArrayDeque<ExecutionGroup>()

    protected var currentExecutionGroup: ExecutionGroup? = null

    var isExecutingBatch by mutableStateOf(false)

    protected inner class BatchCommandBuilder(
    ...
    protected val positionHistory = ArrayDeque<RobotState>(500)

    var isPaused by mutableStateOf(false)

    protected var pauseState: RobotState? = null

    protected var pausePathIndex: Int = -1
    // 必须先见到「运行」再把「停止」当成结束，否则上传/Start 前的空闲状态会把暂停按钮清掉

    protected var programHasStarted = false
    ...
    protected fun sendServoCartCommand() {
        // Calculate Inputs
    ...
    protected val currentActiveWeldPath: WeldPath?
        get() {
    ...
    protected fun updateCurrentActiveWeldPath(newPath: WeldPath) {
    ...
    protected fun applyLoaded(paths: List<WeldPath>, loaded: Map<UUID, WeldProcess>) {
        paths.forEach { path ->
    ...
    protected fun String.toUuidOrNull(): UUID? = try {
        UUID.fromString(trim())
    } catch (_: Exception) {
        null
    }

    protected open fun bindPouchProcesses(paths: List<WeldPath>): Boolean {
    ...
    protected fun encryptMac(mac: String): String {
        try {
    ...
    protected fun getPathDirectionVector(path: WeldPath, cornerPoint: Point3D): Point3D? {
    ...
    open fun selectPoint(weldPathIndex: Int, pointIndex: Int) {
    ...
    protected fun sendToolCoordCommand(toolIndex: Int, pose: Pose) {
    ...
    protected open fun appendCommandsForPath(
        weldPath: WeldPath, 
    ...
    open fun sendBatchMoveLCommands(resumePathIndex: Int = -1, resumePointIndex: Int = -1, isResumeFromStop: Boolean = false) {
    ...
    protected fun getCircleParams(p1: Vector3, p2: Vector3, p3: Vector3): Triple<Vector3, Vector3, Double>? {
        val v1 = p2 - p1
        val v2 = p3 - p2
    ...
    protected fun calculateArcOffset(
        midPoint: WeldPoint,
        endPoint: WeldPoint,
        allPoints: List<WeldPoint>,
        midIndex: Int,
    ...
    protected fun distance(p1: Pose, p2: Pose): Double {
        val dx = p1.x - p2.x
        val dy = p1.y - p2.y
        val dz = p1.z - p2.z
    ...
    protected fun findRetreatPoint(currentState: RobotState): RobotState {
    ...
    protected fun findClosestPathSegment(pose: Pose): Pair<Int, Int> {
    ...
    protected fun findClosestPointIndexOnPath(path: WeldPath, pose: Pose): Int {
        var minDesc = Double.MAX_VALUE
        var bestPoint = -1
        val currentPos = Vector3(pose.x, pose.y, pose.z)
    ...
    protected fun distancePointToSegment(p: Vector3, a: Vector3, b: Vector3): Double {
        val ab = b - a
        val ap = p - a
    ...
    protected fun calculateSegmentLength(points: List<WeldPoint>, fromIndex: Int, toIndex: Int): Double {
    ...
    protected fun calculateArcPart(pStart: WeldPoint, pMid: WeldPoint, pEnd: WeldPoint, isFirstHalf: Boolean): Double {
    ...
    protected fun isSafePoint(point: WeldPoint): Boolean {
        return point.type == WeldPointType.START_SAFE || point.type == WeldPointType.END_SAFE
    }

    protected fun calculateNewArcMidPoint(pStart: Pose, pMid: Pose, pEnd: Pose, pCurrent: Pose): Pose {
    ...
    protected fun interpolateJoints(jCurrent: List<Double>, jEnd: List<Double>): List<Double> {
        if (jCurrent.size < 6 || jEnd.size < 6) return jCurrent
        return List(6) { i -> (jCurrent[i] + jEnd[i]) / 2.0 }
    }

    protected fun angleBetween(v1: Vector3, v2: Vector3, normal: Vector3): Double {
        val dot = v1.dot(v2)
    ...
    protected fun startWireFeed() {
        sendManualCommand(268, "SetForwardWireFeed(0,1)")
    }

    protected fun stopWireFeed() {
        sendManualCommand(268, "SetForwardWireFeed(0,0)")
    }

    protected fun dragTeachSwitch(enable: Boolean) {
    ...
    protected var currentDownloadFileName: String? = null

    protected var pollingJob: kotlinx.coroutines.Job? = null

    protected fun startDownloadPolling(id: Long) {
        pollingJob?.cancel()
    ...
    protected fun checkDownloadStatus(id: Long): Int {
    ...
    protected val downloadReceiver = object : BroadcastReceiver() {
    ...
    protected fun registerDownloadReceiver() {
    ...
    /** 一次作业跑完后的收尾。 */
    protected open fun onRunFinished() {
        markPouchWelding(false)
    }
